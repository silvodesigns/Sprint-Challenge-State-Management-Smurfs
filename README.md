...
- [ ] What problem does the context API help solve?
   ...
   ```` It tries to make state management better. By eliminating the need to pass data around by prop drilling.````
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?

````Kinda like  context  API. It makes a store where we can have all our data. I think of the store as the warehouse like Amazon where all the boxes are then the actions are like customers who order from the warehouse and the reducers are the delivery drivers that take the boxes from the store to the customer house which is the state````
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?

````Application state is available to entire aplication globably and component state is just a piece of that state like maybe props that corncerns dat single component````
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
````It allows us to make asyncrunous calls and perform different actions according to the response ````
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
````Probably Context API because it is kinda like Redux but simpler```
